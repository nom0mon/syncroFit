...
class CommunityController extends Controller
{
    ...
    {
        $posts = Post::query()
            ->with(['user:id,username,avatar_path,updated_at', 'media'])
    ...
    {
        $comments = $post->comments()->with('user:id,username,avatar_path,updated_at')
    ...
    public function comment(Request $request, Post $post): JsonResponse
    {
        ...
        $comment->load('user:id,username,avatar_path,updated_at');
    ...
    {
        $post->load(['user:id,username,avatar_path,updated_at', 'media'])->loadCount(['likes', 'comments']);
        $post->setAttribute('is_liked_by_current_user', $post->likes()->where('user_id', $request->user()->id)->exists());
        if ($comments) {
            $post->load(['comments' => fn ($query) => $query->with('user:id,username,avatar_path,updated_at')->oldest('created_at')->oldest('id')]);
        }
