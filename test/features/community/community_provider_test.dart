...
void main() {
  Post post({bool liked = false, int likes = 0}) => Post(
        id: '1', authorName: 'Alex', content: 'Update',
        timestamp: DateTime.utc(2026, 9, 6), likeCount: likes,
        isLikedByCurrentUser: liked, comments: const [],
      );

  test('failed optimistic like rolls back and duplicate rapid tap is ignored', () async {
    ...
    expect(container.read(communityProvider).value!.posts.single.isLikedByCurrentUser, isFalse);
  });
  ...
  test('created posts are inserted at the top', () async {
    ...
    expect(await container.read(communityProvider.notifier).createPost('New post'), isNull);
    expect(container.read(communityProvider).value!.posts.first.content, 'New post');
  });
  ...
  test('photo uploads are forwarded when a post is created', () async {
    ...
    addTearDown(container.dispose);
    await container.read(communityProvider.future);
    final photo = CommunityPhotoUpload(bytes: Uint8List.fromList([1, 2, 3]), filename: 'photo.jpg');

    expect(await container.read(communityProvider.notifier).createPost('', photos: [photo]), isNull);
...
class _FakeCommunityRepository implements CommunityRepository {
  ...
  @override
  Future<Result<Post, AppError>> getPostById(String id) async => Success(initial);
  @override
  Future<Result<Post, AppError>> createPost(Post post, {List<CommunityPhotoUpload> photos = const [], void Function(int, int)? onProgress}) async {
    ...
    return Success(Post(
        id: '2', authorName: 'You', content: post.content, timestamp: DateTime.now(),
  ...
  @override
  Future<Result<Post, AppError>> addComment(String postId, Comment comment) async => Success(initial);
  @override
  Future<Result<void, AppError>> deletePost(String postId) async => const Success(null);
  @override
  Future<Result<void, AppError>> deleteComment(String postId, String commentId) async => const Success(null);
}
